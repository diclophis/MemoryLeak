...
DhcpNameServer
  ...
  //  printf("IP: %s\n", inet_ntoa(*((struct in_addr *)&_res.nsaddr_list[i])) ); 
  //}

  //char **hp = &_res.nsaddr_list;
...
struct __res_state myres = _res;
status = ARES_SUCCESS;
for(index=0; index< myres.nscount;index++){
  if (status == ARES_SUCCESS) {
    struct sockaddr_in add = myres.nsaddr_list[index];
    char *dnsname = inet_ntoa(add.sin_addr);
    printf("Adding nameserver: %s\n", dnsname);
    status = config_nameserver(&servers, &nservers, dnsname);
  }
}
